...
public class ModBlockStateModelProvider extends BlockStateProvider {
    ...
    private void buildSimpleOreBlock(Block oreBlock, String basePath) {
        simpleBlock(oreBlock, ConfiguredModel.builder()
                .modelFile(buildModelFile(
                        ((IGeoBlock) oreBlock).getGeologyType(),
                        ((IGeoBlock) oreBlock).getOreType(),
                        ((IGeoBlock) oreBlock).getGradeType(),
                        basePath))
                .build());
    }
    ...
    private ModelFile buildModelAllSides(String basePath, OreType oreType, GradeType gradeType) {
        ...
        if (!oreType.hasOre()) {
            modelFile = models().withExistingParent("block/" + basePath,
                            mcLoc("block/cube"))
            ...
            String oreName = oreType.name();
            String gradeName = gradeType.name();

            modelFile = models().withExistingParent("block/blockore/"
                                    + basePath + "/" + oreName + "/" + gradeName,
